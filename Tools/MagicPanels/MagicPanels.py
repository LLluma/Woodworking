...
def getVertexIndex(iObj, iVertex):
    ...
    for v in ves:
        if equal(v.X, iVertex.X) and equal(v.Y, iVertex.Y) and equal(v.Z, iVertex.Z):
            return index
...
def getOnCurve(iPoint, iCurve):
    ...
    for v in curvePoints:
        if equal(v.x, targetVector.x) and equal(v.y, targetVector.y) and equal(v.z, targetVector.z):
            return index
    ...
    try:
        ...
        if depth == 0:
            ...
            while i < depth and i < gSearchDepth:
                ...
                if base.isDerivedFrom("Part::Box") or (
...
def getModelRotation(iX, iY, iZ):
    ...
    [
        ...
        py,
        pz,
    ] = FreeCADGui.ActiveDocument.ActiveView.viewPosition().Rotation.getYawPitchRoll()
...
def getDirection(iObj):
    ...
    if iObj.isDerivedFrom("Part::Box"):
        if (
            ...
            and iObj.Height.Value < iObj.Length.Value
            and iObj.Width.Value <= iObj.Length.Value
        ):
            return "XY"
        ...
        if (
            iObj.Width.Value < iObj.Height.Value
            and iObj.Width.Value < iObj.Length.Value
            and iObj.Height.Value <= iObj.Length.Value
        ):
            return "XZ"
        ...
        if (
            iObj.Length.Value < iObj.Height.Value
            and iObj.Length.Value < iObj.Width.Value
            and iObj.Height.Value <= iObj.Width.Value
        ):
            return "YZ"
...
def getContainersOffset(iObj):
    ...
    for o in containers:
        if (
            ...
            or o.isDerivedFrom("App::LinkGroup")
...
def moveToClean(iObjects, iSelection):
    ...
    # if containers
        ...
        while i < rsize and i < gSearchDepth:
            ...
            if c.isDerivedFrom("App::LinkGroup") or c.isDerivedFrom("App::DocumentObjectGroup"):
...
def moveToFirst(iObjects, iSelection):
    ...
    while i < rsize and i < gSearchDepth:
        ...
        elif c.isDerivedFrom("App::Part") or c.isDerivedFrom("PartDesign::Body"):
...
def getObjectToMove(iObj):
    ...
    if (
        iObj.isDerivedFrom("Part::Box")
        or iObj.isDerivedFrom("Part::Cut")
        or iObj.isDerivedFrom("PartDesign::Body")
        or iObj.isDerivedFrom("App::LinkGroup")
    ):
        return iObj
...
def makePad(iObj, iPadLabel="Pad"):
    ...
    geoList.append(
        Part.LineSegment(
            FreeCAD.Vector(115.695488, 159.435455, 0),
            FreeCAD.Vector(274.784485, 159.435455, 0),
        )
    )
    geoList.append(
        Part.LineSegment(
            FreeCAD.Vector(274.784485, 159.435455, 0),
            FreeCAD.Vector(274.784485, 53.166523, 0),
        )
    )
    geoList.append(
        Part.LineSegment(
            FreeCAD.Vector(274.784485, 53.166523, 0),
            FreeCAD.Vector(115.695488, 53.166523, 0),
        )
    )
    geoList.append(
        Part.LineSegment(
...
def makeHoles(iObj, iFace, iCylinders):
    ...
    for o in objects:
        ...
        holeSketch.renameConstraint(1, "Hole00Diameter")
...
def makeCountersinks(iObj, iFace, iCones):
    ...
    for o in objects:
        ...
        holeSketch.renameConstraint(3, "Countersink00Diameter")
...
def makeCounterbores(iObj, iFace, iCones):
    ...
    for o in objects:
        ...
        holeSketch.addConstraint(Sketcher.Constraint("Diameter", 1, r2))
        holeSketch.setDatum(3, FreeCAD.Units.Quantity(sr2))
        holeSketch.renameConstraint(3, "Counterbore00Diameter")
...
def makePocketHoles(iObj, iFace, iCones):
    ...
    for o in objects:
        ...
        holeSketch.addConstraint(Sketcher.Constraint("Diameter", 0, r1))
        holeSketch.setDatum(1, FreeCAD.Units.Quantity(sr1))
        holeSketch.renameConstraint(1, "Tip0hole00Diameter")
        ...
        holeSketch.addConstraint(Sketcher.Constraint("Diameter", 1, r2))
        holeSketch.setDatum(3, FreeCAD.Units.Quantity(sr2))
        holeSketch.renameConstraint(3, "Pocket0hole00Diameter")
...
def makeCounterbores2x(iObj, iFace, iCones):
    ...
    for o in objects:
        ...
        holeSketch1.renameConstraint(1, "Hole100Diameter")
        ...
        holeSketch1.renameConstraint(3, "Counterbore100Diameter")
        ...
        holeSketch2.renameConstraint(1, "Hole200Diameter")
        ...
        holeSketch2.renameConstraint(3, "Counterbore200Diameter")
    ...
    for o in objects:
        ...
        if not hasattr(copy, "BOM"):
            info = QT_TRANSLATE_NOOP(
                "App::Property",
                "Allows to skip this duplicated copy in BOM, cut-list report.",
            )
...
def copyColors(iSource, iTarget):
    ...
    # handling links
    if (
        iSource.isDerivedFrom("App::LinkGroup")
        or iTarget.isDerivedFrom("App::LinkGroup")
        or iSource.isDerivedFrom("App::Link")
        or iTarget.isDerivedFrom("App::Link")
    ):
        # normal -> link
        try:
