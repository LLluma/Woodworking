...
def getDebugInfo():
    ...
    if error == 1:
        try:
            info = translate(
                "debugInfo",
                "This FreeCAD version is too buggy to get debug information.",
            )
...
def showQtGUI():
    class QtMainClass(QtGui.QDialog):
        ...
        def initUI(self):
            ...
            try:
                ...
                if gWBCurrent["info"] == "3":
                    ...
                    info += translate(
                        "debugInfo",
                        "Consider upgrade workbench to the new version branch",
                    )
            ...
            try:
                if gTests["status"] == "":
                    ...
                    info += translate(
                        "debugInfo",
                        "All tests passed. This FreeCAD version is safe to use.",
                    )
                else:
                    info += self.getIcon("no", iconSize, iconAlign)
            ...
            try:
                if gWBCurrent["Certified"] == "yes":
                    ...
                    info += translate(
                        "debugInfo",
                        "Your FreeCAD version not match the Woodworking workbench version. ",
                    )
            except:
                skip = 1
            ...
            try:
                if gWBCurrent["update"] == "update":
                    self.ub1 = QtGui.QPushButton(
                        translate("debugInfo", "update workbench \n and restart FreeCAD"),
                        self,
                    )
                if gWBCurrent["update"] == "upgrade":
                    self.ub1 = QtGui.QPushButton(
                        translate("debugInfo", "upgrade workbench \n and restart FreeCAD"),
                        self,
                    )
        ...
        def wbUpdate(self):
            ...
            info += translate(
                "debugInfo",
                "Latest update for Woodworking workbench will be downloaded. ",
            )
            info += translate(
                "debugInfo",
                "FreeCAD will restart with new Woodworking workbench version ",
            )
