...
def showQtMain():
    ...
    class QtMainClass(QtGui.QDialog):
        ...
        def initUI(self):
            ...
            # info screen
            self.eTypeIS = QtGui.QLabel(
                translate("sheet2export", "all spreadsheets") + "                  ",
                self,
            )
            ...
            # info screen
            self.fileTypeOIS = QtGui.QLabel(
                translate("sheet2export", "HyperText Markup Language (.html file)") + "          ",
                self,
            )
            ...
            # text input label
            self.customCSStil = QtGui.QLabel(
                translate(
                    "sheet2export",
                    "Step 2. Custom CSS rules for each cell (edit or add):",
                ),
                self,
            )
    ...
    if sQT == "yes":
        if int(dbMaxR * dbMaxC) > 10000:
            ...
            info += (
                translate(
                    "sheet2export",
                    "Would you like to wait (ok) or skip (cancel) the spreadsheet?",
                )
                + "\t\t"
            )
            info += "\n\n"
